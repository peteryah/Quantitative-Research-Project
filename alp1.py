...
class Alphas(object):
    def __init__(self, df_data):
        ...
        self.returns = df_data['pctr'] 
        self.vwap = (df_data['amount'])/(df_data['volume']) 
        
    
    def alpha001(self):
        '''Alpha#1	 (rank(Ts_ArgMax(SignedPower(((returns < 0) ? stddev(returns, 10) : close), 2.), 5)) -0.5)'''
        inner = self.close
        inner[self.returns < 0] = stddev(self.returns, 10)
    ...
    def alpha007(self):
        '''Alpha#7	 ((adv10 < volume) ? ((-1 * ts_rank(abs(delta(close, 7)), 60)) * sign(delta(close, 7))) : (-1* 1))'''
        adv20 = sma(self.volume, 10)
    ...
    def alpha021(self):
        ...
        cond_2 = sma(self.volume, 10) / self.volume < 1
...
#                             columns=self.close.columns)
    ...
    def alpha022(self):
        ...
        df = df.replace([-np.inf, np.inf], 0).fillna(value=0)
        return -1 * delta(df, 5) * rank(stddev(self.close, 10))

    def alpha023(self):
        '''Alpha#23	 (((sum(high, 20) / 20) < high) ? (-1 * delta(high, 2)) : 0)'''
        cond = sma(self.high, 10) < self.high
    ...
    def alpha030(self):
        ...
        return ((1.0 - rank(inner)) * ts_sum(self.volume, 5)) / ts_sum(self.volume, 10)
    ...
    def alpha041(self):
        ...
        return pow((self.high * self.low),0.5) - self.close
    ...
    def alpha043(self):
        ...
        adv20 = sma(self.volume, 10)
        return ts_rank(self.volume / adv20, 10) * ts_rank((-1 * delta(self.close, 7)), 8)
    ...
    def alpha045(self):
        ...
        df = df.replace([-np.inf, np.inf], 0).fillna(value=0)
        return -1 * (rank(sma(delay(self.close, 5), 10)) * df *
                     rank(correlation(ts_sum(self.close, 5), ts_sum(self.close, 20), 2)))
    
    def alpha046(self):
        '''Alpha#46	 ((0.25 < (((delay(close, 20) - delay(close, 10)) / 10) - ((delay(close, 10) - close) / 10))) ?(-1 * 1) : (((((delay(close, 20) - delay(close, 10)) / 10) - ((delay(close, 10) - close) / 10)) < 0) ? 1 :((-1 * 1) * (close - delay(close, 1)))))'''
        inner = ((delay(self.close, 10) - delay(self.close, 5)) / 5) - ((delay(self.close, 5) - self.close) / 5)
    ...
    def alpha052(self):
        ...
        return (((-1 * delta(ts_min(self.low, 5), 5)) *
                 rank(((ts_sum(self.returns, 240) - ts_sum(self.returns, 10)) / 220))) * ts_rank(self.volume, 5))
...
#        return (0 - (1 * (rank((sma(self.returns, 10) / sma(sma(self.returns, 2), 3))) * rank((self.returns * self.cap)))))
    ...
    def alpha057(self):
        ...
        return (0 - (1 * ((self.close - self.vwap) / decay_linear(rank(ts_argmax(self.close, 15)).to_frame(), 2).CLOSE)))
    ...
    def alpha077(self):
        '''Alpha#77	 min(rank(decay_linear(((((high + low) / 2) + high) - (vwap + high)), 20.0451)),rank(decay_linear(correlation(((high + low) / 2), adv40, 3.1614), 5.64125)))'''
        adv40 = sma(self.volume, 40)
        p1=rank(decay_linear(((((self.high + self.low) / 2) + self.high) - (self.vwap + self.high)).to_frame(), 10).CLOSE)
    ...
    def alpha078(self):
        '''Alpha#78	 (rank(correlation(sum(((low * 0.352233) + (vwap * (1 - 0.352233))), 19.7428),sum(adv40, 19.7428), 6.83313))^rank(correlation(rank(vwap), rank(volume), 5.77492)))'''
        adv40 = sma(self.volume, 40)
        return (rank(correlation(ts_sum(((self.low * 0.352233) + (self.vwap * (1 - 0.352233))), 10),ts_sum(adv40,20), 7)).pow(rank(correlation(rank(self.vwap), rank(self.volume), 6))))
